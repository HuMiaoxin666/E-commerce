...
(function() {
  ...
  Sankey = (function() {
    function Sankey() {
      ...
      this.box_width = 20;
      this.flow_edge_width = 1;
      this.flow_curve = 0.25;
      this.boxes = {};
      this.box_array = [];
	  this.all_height = 139.9;
      this.lines = {};
      this.line_array = [];
      this.stacks = [];
      this.bubbles = [];
      this.bubbleColor = '#000';
      this.color_dp = d3.scale.category20();
	    this.color = d3.scaleOrdinal().domain([0, 1, 2, 3, 4, 5,6]).range(['#00FF00', '#62ffd0', '#F4A6FF', '#ffd062', '#4D5559', '#FFFFE9', '#FB7501']);

      this.bubbleLabelColor = '#fff';
      this.opacity = '.6';
      this.opacity_highlight = '0.1';
	  this.index = 0;
	  this.ch_int = ["农林牧渔","采掘","食品制造","纺织","其他制造","电力","炼焦","化学","非金属","金属","机械","建筑","运输","贸易","房地产","金融","其他服务"]
    }
    ...
    Sankey.prototype.calculateXStep = function(){
      ...
      return (this.display_width - this.left_margin - this.right_margin - 20) / maximum_x;
    };
...
//******************改变box位置*************************
    Sankey.prototype.position_boxes_and_lines = function(){
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        if(_i == 0)
        {
          ...
          var arr = [];
          var cur_space = (175 -cur_height )/cur_box;
          var t_b_ind = 1;
          for (_j = 0, _len1 = _ref2.length; _j < _len1; _j++) {
            name = _ref2[_j];
            box = this.boxes[name];
      ...
      //console.log(new_p_arr); 
      //计算label_value
      var all_arr = [];
      //***********开始时从左到右进行变换位置***********
      console.log("check");
      for(var s = 1; s < new_p_arr.length; s++)
      {
        //计算从第二列开始的值
          var box_names = new_p_arr[s];
          var temp_arr = new Array(box_names.length);
          for(var b = 0; b < box_names.length; b++)
          {
            temp_arr[b] = {};
            temp_arr[b].name = box_names[b];
            temp_arr[b].index_value = b+1;
            var lf_lines = this.boxes[box_names[b]].left_lines;
            temp_arr[b].w = 0;
            var cur_w_all = 0;
            for(var line = 0 ; line < lf_lines.length; line++){
              cur_w_all += lf_lines[line].flow;
            }

            for(var line = 0 ; line < lf_lines.length; line++){
              temp_arr[b].w += lf_lines[line].flow*(new_p_arr[s-1].indexOf(lf_lines[line].left_box.name)+1);
            }
            temp_arr[b].w = temp_arr[b].w/cur_w_all;       
          }
          all_arr[s] = temp_arr;
          //对每列进行冒泡排序
          console.log(all_arr[s]);
          for(var b = 0; b < all_arr[s].length -1; b++)
          {
            for(var i = b+1;i<all_arr[s].length;i++)
            {
              
              if(all_arr[s][b].w > all_arr[s][i].w)
              {
                //改变box的位置
                this.boxes[all_arr[s][i].name].y =  this.boxes[all_arr[s][b].name].y;
                for(var k = b ; k < i; k++){
                  this.boxes[all_arr[s][k].name].y += this.boxes[all_arr[s][i].name].size()  -this.boxes[all_arr[s][b].name].size();
                }
                this.boxes[all_arr[s][b].name].y = this.boxes[all_arr[s][i-1].name].y + this.boxes[all_arr[s][i-1].name].size()+this.boxes[all_arr[s][i-1].name].cur_space;
                var temp = all_arr[s][i];
                var temp_index_i = all_arr[s][i].index_value;
                var temp_index_b = all_arr[s][b].index_value;
                all_arr[s][i] = all_arr[s][b];
                all_arr[s][i].index_value = temp_index_i;
                all_arr[s][b] = temp;
                all_arr[s][b].index_value = temp_index_b;

                var temp_name = new_p_arr[s][i];
                new_p_arr[s][i] = new_p_arr[s][b]
                new_p_arr[s][b] = temp_name;
              }
            }
          }
      }
      ...
      //***********从右到左进行变换位置***********
      for(var s = all_arr.length - 2 ; s >=0 ; s--)
      {
        //给第一列的部门新建一个数组赋值名称
        if(s == 0){
          var temp_arr = new Array(new_p_arr[s].length);
          for(var b = 0; b < new_p_arr[s].length; b++)
          {
            temp_arr[b] = {};
            temp_arr[b].name = new_p_arr[s][b];
          }
          all_arr[s] = temp_arr;
        }
        //根据右边的计算左边的值
        for(var b = 0 ; b < all_arr[s].length; b++)
        {
          var ri_lines = this.boxes[all_arr[s][b].name].right_lines;
          all_arr[s][b].w = 0;
          var cur_w_all = 0;
          for(var line = 0 ; line < ri_lines.length; line++){
            cur_w_all += ri_lines[line].flow;
          }
          for(var line = 0 ; line < ri_lines.length; line++){
            all_arr[s][b].w += ri_lines[line].flow*(new_p_arr[s+1].indexOf(ri_lines[line].right_box.name)+1);
          }
          all_arr[s][b].w = all_arr[s][b].w/cur_w_all;     
        }
        //对左边的box进行冒泡排序
        for(var b = 0; b < all_arr[s].length -1; b++)
        {
          for(var i = b+1;i<all_arr[s].length;i++)
          {
              // console.log('all_arr[s][i].w: ', all_arr[s][i].w);
              // console.log('all_arr[s][b].w: ', all_arr[s][b].w);
              if(all_arr[s][b].w > all_arr[s][i].w)
              {
                //改变box的位置
                this.boxes[all_arr[s][i].name].y =  this.boxes[all_arr[s][b].name].y;
    ...
    Sankey.prototype.fade_unless_highlighted = function() {
	  ...
	  var ch_int = ["农林牧渔","采掘","食品制造","纺织","其他制造","电力","炼焦","化学","非金属","金属","机械","建筑","运输","贸易","房地产","金融","其他服务"]
	function judge_first_col(name){
		var bool = false;
		for(var i = 0; i< 17;i++)
		{
			if(name == ch_int[i])
	  ...
	  {
		  if(judge_first_col(_ref1[i].name))
			  console.log(_ref1[i].name);
	  }
	  */
      _results = [];
      for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
  ...
  FlowLine = (function() {
    function FlowLine(sankey, left_box_name, flow, right_box_name,department) {
    ...
    FlowLine.prototype.labelText = function() {
      return this.sankey.convert_flow_labels_callback(this.department);
    };
    ...
    FlowLine.prototype.draw = function(r) {
      ...
      this.left_label = r.text(this.ox - 6*(this.department.length), this.oy - (this.size / 5), this.labelText()).attr({
        'text-anchor': 'start'
      });
      this.right_label = r.text(this.dx + 6*(this.department.length), this.dy - (this.size / 5), this.labelText()).attr({
        'text-anchor': 'end'
      });
    ...
    FlowLine.prototype.redraw = function(r) {
      ...
      this.left_label.attr({
        text: this.labelText(),
        x: this.ox - 6*(this.department.length),
        y: this.oy - (this.size / 5)
      });
      return this.right_label.attr({
        text: this.labelText(),
        x: this.dx + 6*(this.department.length),
        y: this.dy - (this.size / 5)
      });
    };
    ...
    FlowLine.prototype.fade_unless_highlighted = function() {
	  ...
	  /*
	  if(this.outer_line.department == this.depatment)
	  {
		  
		  return this.outer_line.attr({
			'opacity':function(d){
				console.log(d);
				return this.sankey.opacity_highlight;
			} 
		  });
	  }
	  else {*/
  ...
  TransformationBox = (function() {
    ...
    TransformationBox.prototype.size = function(){
	  ...
	  var right_lines_height = 0;
	  if(this.right_lines!= null)
	  {
		  right_lines = this.right_lines;
		  for (_i = 0, _len = right_lines.length; _i < _len; _i++) {
			line = right_lines[_i];
			if (line.size > this.sankey.threshold_for_drawing) {
				right_lines_height += line.size;
			}
		  }
		 // console.log("**********right_lines_height*****:"+right_lines_height);
	  }
	  ...
	  if(right_lines_height>s)
		  s = right_lines_height;
    ...
    TransformationBox.prototype.position_and_colour_lines = function(){
	 ...
	 //**********计算左边边总长度**************
	  ...
	  var left_height = 0;
	  var left_lines_count = 0;
	  for (_i = 0, _len = left_lines.length; _i < _len; _i++){
			var line_test = left_lines[_i];
			left_height += line_test.size;
			left_lines_count++;
	  }
	  //console.log(this.sankey.all_height);
	
	  //console.log("left_lines_count: "+left_lines_count);
	 // console.log("left_height: "+left_height);
	  
	  //*************计算右边边总长度**************
	 
	  var right_height = 0;
	  var right_lines_count = 0;
	  for (_j = 0, _len1 = right_lines.length; _j < _len1; _j++){
			line_test = right_lines[_j];
			right_height += line_test.size;
			right_lines_count++;
	 }
	  //console.log("right_lines_count: "+right_lines_count);
	  ...
	  {
		  var sub_value = right_height - left_height;
		  var add_y = sub_value/right_lines_count;
		   for (_i = 0, _len = left_lines.length; _i < _len; _i++) {
			   if(_i == 0)
			   {
				   	line = left_lines[_i];
				...
				else
				{
					line = left_lines[_i];
					line.dx = this.x;
					line.dy = ly + (line.size/2)+add_y;
					ly = ly + line.size+add_y;
				}
		  }
	  }
	  ...
	  else if(left_height > right_height)
	  {
		   ...
		   for (_j = 0, _len1 = right_lines.length; _j < _len1; _j++) {
				...
				line.ox = this.x + box_width;
				line.oy = ry + (line.size / 2) -add_y;
				_results.push(ry = ry + line.size-add_y);
		  }
	  }
    ...
    TransformationBox.prototype.bubbleColourForValue = function() {
      if (this.bubbleValue > 0) {
		console.log(this.sankey.color(this.bubbleValue));
        return this.sankey.color(this.bubbleValue);
      }
    ...
    TransformationBox.prototype.draw = function(r) {
      ...
      var cur_year = 0;
      var cur_index ;
      for(var i = 0; i<this.sankey.stacks.length; i++)
      {
        cur_index = this.sankey.stacks[i].box_names.indexOf(this.name);
        if(cur_index != -1)
          if(i == 0)
            cur_color = this.sankey.color_dp(cur_index);
          else{           
            cur_color = this.sankey.color(cur_index);
            cur_year = i;
          }
      }
    this.sankey.index++;
