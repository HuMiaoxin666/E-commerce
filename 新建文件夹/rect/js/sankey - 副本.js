  ...
  Sankey = (function() {
    function Sankey() {
      ...
      this.color_dp = d3.scale.category20();
	    this.color = d3.scaleOrdinal().domain([0, 1, 2, 3, 4, 5,6]).range(['#F9726F', '#F4CD29', '#FBA175', '#AA789F', '#49A1BC', '#A9D18E', '#6E797A']);
...
//******************改变box位置*************************
    Sankey.prototype.position_boxes_and_lines = function(){
      ...
      //计算label_value
      this.boxes[_ref[0].box_names[0]].y = 100; 
      var all_arr = [];
      ...
      var step = 0;
      for(var s = 0; s < all_arr.length; s++)
      {
        if(all_arr[s] != null)
        {
          //冒泡排序
          for(var b = 0; b < all_arr[s].length -1; b++)
          {
            for(var i = b+1;i<all_arr[s].length;i++)
            {
              ...
              {
                var y = this.boxes[all_arr[s][i].name].y;
                this.boxes[all_arr[s][i].name].y =  this.boxes[all_arr[s][b].name].y;
                this.boxes[all_arr[s][b].name].y = y - this.boxes[all_arr[s][b].name].size()+ this.boxes[all_arr[s][i].name].size();
                console.log(all_arr[s][b].name,all_arr[s][i].name);
      ...
      /*
      for(var i = all_arr.length - 2 ; i >=0 ; i--)
      {
        //给第一列的部门新建一个数组赋值名称
        if(i == 0){
          var temp_arr = new Array(new_p_arr[i].length);
          for(var b = 0; b < new_p_arr[i].length; b++)
          {
            temp_arr[b] = {};
            temp_arr[b].name = new_p_arr[i][b];
          }
          all_arr[i] = temp_arr;
        }
        
        for(var b = 0 ; b < all_arr[i].length; b++)
        {
          var ri_lines = this.boxes[all_arr[i][b].name].right_lines;
          all_arr[i][b].w = 0;
          var cur_w_all = 0;
          ...
          for(var line = 0 ; line < ri_lines.length; line++){
            all_arr[i][b].w += ri_lines[line].flow*(new_p_arr[i+1].indexOf(ri_lines[line].left_box.name)+1);
          }
          all_arr[i][b].w = all_arr[i][b].w/cur_w_all;     
        }
      }
      for(var s = 0 ; s < all_arr.length - 1; s++)
      {
        //冒泡排序
        for(var b = 0; b < all_arr[s].length -1; b++)
        {
  ...
  TransformationBox = (function() {
    ...
    TransformationBox.prototype.draw = function(r) {
      ...
      {
        var cur_index = this.sankey.stacks[i].box_names.indexOf(this.name);
        if(cur_index != -1)
          if(i == 0)
